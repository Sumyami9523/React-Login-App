...
function UpdateCustomer(){
    ...
    const validateAndUpdate=async(e)=>{
        e.preventDefault();
            ...
            try {
                const response = await axios.get(`http://localhost:3000/checkUniqueCustNum/${custNum}`);
        ...
        axios.put('http://localhost:3000/updateCustomer/'+id,{custName,custNum,city,state,pincode})
        .then(result=>{
            console.log(result)
            navigate('/home')
    ...
    useEffect(()=>{
        axios.get('http://localhost:3000/getCustomer/'+id)
        .then(result=>{
            console.log(result)
            setCustName(result.data.custName)
            setCustNum(result.data.custNum)
            ...
            <div className="w-50 bg-white rounded p-3">
                <form onSubmit={validateAndUpdate}>
                    ...
                    <div className="mb-2">
                        ...
                        <input type='text' placeholder="Enter Customer Number" value={custNum} className="form-control" onChange={(e)=>setCustNum(e.target.value)}/>
                    </div>
                    <div className="mb-2">
